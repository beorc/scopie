...
describe Scopie::Base do
  ...
  describe '#apply_scopes' do
    ...
    context 'given the hash containing one scope value' do
      ...
      it 'should call the apply_scope on subject and return target' do
        expect(subject).to receive(:apply_scope).once do |name, scope, val, params|
          expect(name).to eq(scope_name)
          expect(scope).to eq(target)
          expect(val.coerced).to eq hash[scope_name]
    ...
    context 'given scopie having scope method defined' do
      ...
      let(:value) { Scopie::Value.new(hash, another_scope_name, options) }
      ...
      context 'given the hash containing one scope value' do
        ...
        it 'should call the scopie method' do
          ...
          expect(subject.send(:apply_scope, another_scope_name, target, value, hash)).to eq target
        end
      end
    end
  end
  ...
  describe '#apply_scope' do
    ...
    let(:coerced_value) { '0' }
    let(:hash) { { scope_name => coerced_value, another_scope_name => :another_value } }
    let(:result) { target }

    it 'should call the scope method on target and return target' do
      expect(target).to receive(scope_name).once.with(coerced_value).and_return(target)
    ...
    context 'given true result' do
      ...
      it 'should call the scope method on target and return target' do
        expect(target).to receive(scope_name).once.with(coerced_value).and_return(result)
        expect(subject.send(:apply_scope, scope_name, target, value, hash)).to eq target
      end
    end
  ...
  describe '#current_scopes' do
    context 'given hash' do
      ...
      let(:current_scopes) { subject.current_scopes(hash, only_method_name) }
      ...
      context 'given no method' do
        let(:current_scopes) { subject.current_scopes(hash) }
      ...
      context 'given the except method' do
        let(:current_scopes) { subject.current_scopes(hash, except_method_name) }
